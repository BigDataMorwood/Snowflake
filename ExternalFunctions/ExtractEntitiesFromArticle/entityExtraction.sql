...
with cte_strings as
(
  select EXT_UDF_entity_extraction(bodyText) as results //Column name can't be used inline
  from articleText
)
...
create or replace table mappings_EntityToStockSymbol
(
  entity string,
  entitytype string,
  symbol string
...
insert into mappings_entityToStockSymbol ( entity, entitytype, symbol )
values ('Amazon', 'ORGANIZATION', 'AMZN'),
       ('Amazon Web Services', 'ORGANIZATION', 'AMZN'),
       ('AWS Marketplace', 'ORGANIZATION', 'AMZN'),
       ('Microsoft', 'ORGANIZATION', 'MSFT'),
       ('Microsoft Corp.', 'ORGANIZATION', 'MSFT'),
       ('Azure', 'ORGANIZATION', 'MSFT'),
       ('Google', 'ORGANIZATION', 'GOOGL'),
       ('Splunk', 'ORGANIZATION', 'SPLK'),
       ('MongoDB', 'ORGANIZATION', 'MBG'),
       ('New Relic', 'ORGANIZATION', 'NEWR'),
       ('Alteryx', 'ORGANIZATION', 'AYX'),
       ('Elastic', 'ORGANIZATION', 'ESTC'),
       ('Cloudera', 'ORGANIZATION', 'CLDR'),
       ('Talend', 'ORGANIZATION', 'TLND'),
       ('IBM', 'ORGANIZATION', 'IBM'),
       ('Oracle', 'ORGANIZATION', 'ORCL'),
       ('VMWare', 'ORGANIZATION', 'VMW'),
       ('SAP', 'ORGANIZATION', 'SAP');


--Let's join this with out entities to monitor for!
select * from mappings_entitytostocksymbol;
...
--Extract entities, join with the mapping table
with cte_entityExtraction as
(
  select EXT_UDF_entity_extraction(bodyText) as results
  from articleText
)
...
(
  select f.value as EntityVariant,
         ...
         f.value:Text::string as Entity,
         f.value:Score::number(11, 10) as Score
  from cte_entityExtraction, 
       lateral flatten(input => results:Entities) f 
//   where entitytype = 'ORGANIZATION';
)

--Join with the mapping table
//select distinct f.*, m.Symbol
select distinct m.Symbol
from cte_flattened f
join mappings_entitytostocksymbol m
        on f.entity = m.entity
        and f.entitytype = m.entitytype
;
...
select * from extfuncdemodb.elt.vw_extractEntitiesFromText;